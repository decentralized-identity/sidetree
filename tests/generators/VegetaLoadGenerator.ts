...
export default class VegetaLoadGenerator {
   ...
   */
  public static async generateLoadFiles(
    uniqueDidCount: number,
    endpointUrl: string,
    ...
    for (let i = 0; i < uniqueDidCount; i++) {
      ...
      const [publicKey, privateKey] = await Cryptography.generateKeyPairHex(
        keyId,
        KeyUsage.recovery
      );
      fs.writeFileSync(
        absoluteFolderPath + `/keys/privateKey${i}.json`,
        JSON.stringify(privateKey)
      );
      fs.writeFileSync(
        absoluteFolderPath + `/keys/publicKey${i}.json`,
      ...
      const createPayload = JSON.parse(createOperationBuffer.toString())
        .payload;
      fs.writeFileSync(
        absoluteFolderPath + `/requests/create${i}.json`,
      ...
      fs.writeFileSync(
        absoluteFolderPath + `/requests/update${i}.json`,
    ...
    fs.writeFileSync(
      absoluteFolderPath + '/createTargets.txt',
      createTargetsFileString
    );
    ...
    fs.writeFileSync(
      absoluteFolderPath + '/updateTargets.txt',
      updateTargetsFileString
    );
  }
}
