...
describe('SpendingMonitor', () => {
  ...
  const bitcoinFeeSpendingCutoffInSatoshis =
    3 * BitcoinProcessor['satoshiPerBitcoin'];
  const mockTxns: TransactionModel[] = [
    // tslint:disable-next-line: max-line-length
    {
      transactionNumber: 12345,
      transactionTime: 10,
      transactionTimeHash: 'hash1',
      anchorString: 'anchor_string1',
      transactionFeePaid: 100,
      normalizedTransactionFee: 90
    },
    // tslint:disable-next-line: max-line-length
    {
      transactionNumber: 67890,
      transactionTime: 11,
      transactionTimeHash: 'hash2',
      anchorString: 'anchor_string2',
      transactionFeePaid: 110,
      normalizedTransactionFee: 95
    }
  ];

  beforeEach(() => {
    spendingMonitor = new SpendingMonitor(
      bitcoinFeeSpendingCutoffPeriodInBlocks,
      bitcoinFeeSpendingCutoffInSatoshis,
  ...
  describe('isCurrentFeeWithinSpendingLimit', () => {
    it('should return false if the spending limit is reached for the cutoff period of 1', async done => {
      ...
      const result = await spendingMonitor.isCurrentFeeWithinSpendingLimit(
        bitcoinFeeSpendingCutoffInSatoshis + 1,
        5
      );
    ...
    it('should return false if the spending limit is reached.', async done => {
      ...
      const fees = [
        bitcoinFeeSpendingCutoffInSatoshis / 3,
        bitcoinFeeSpendingCutoffInSatoshis / 3,
        bitcoinFeeSpendingCutoffInSatoshis / 3 + 1000
      ];

      mockTxns[0].transactionFeePaid = fees[0];
      mockTxns[1].transactionFeePaid = fees[1];

      const txnStoreSpy = spyOn(
        spendingMonitor['transactionStore'],
        'getTransactionsLaterThan'
      );
      txnStoreSpy.and.returnValue(Promise.resolve(mockTxns));

      const filterTxnSpy = spyOn(
        spendingMonitor as any,
        'findTransactionsWrittenByThisNode'
      );
      ...
      const expectedTxnNumberForTxnStore =
        TransactionNumber.construct(
          lastProcessBlockHeight - bitcoinFeeSpendingCutoffPeriodInBlocks - 2,
          0
        ) - 1;
      expect(txnStoreSpy).toHaveBeenCalledWith(
        expectedTxnNumberForTxnStore,
        undefined
      );
      done();
    });
