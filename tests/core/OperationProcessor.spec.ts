...
async function createUpdateSequence (
  ...
  for (let i = 0; i < numberOfUpdates; ++i) {
    ...
    // Now that the update payload is created, update the update commitment/reveal for the next operation generation to use.
...
describe('OperationProcessor', async () => {
  ...
  it('should process update to remove a public key correctly', async () => {
    ...
    const nextUpdateCommitmentHash = 'EiD_UnusedNextUpdateOneTimePasswordHash_AAAAAA';
    const updateOperationRequest = await OperationGenerator.createUpdateOperationRequest(
      didUniqueSuffix,
