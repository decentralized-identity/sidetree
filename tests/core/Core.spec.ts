...
describe('Core', async () => {
  ...
  describe('initialize', async () => {
    it('should initialize all required dependencies', async () => {
      ...
      const transactionStoreInitSpy = spyOn(
        core['transactionStore'],
        'initialize'
      );
      const unresolvableTransactionStoreInitSpy = spyOn(
        core['unresolvableTransactionStore'],
        'initialize'
      );
      ...
      const observerStartSpy = spyOn(
        core['observer'],
        'startPeriodicProcessing'
      );
      const batchSchedulerStartSpy = spyOn(
        core['batchScheduler'],
        'startPeriodicBatchWriting'
      );
      const blockchainStartSpy = spyOn(
        core['blockchain'],
        'startPeriodicCachedBlockchainTimeRefresh'
      );
  ...
  describe('handleGetVersionRequest()', async () => {
    it('should call all the dependent services', async () => {
      ...
      const expectedCoreVersion: ServiceVersionModel = {
        name: 'a-service',
        version: 'x.y.z'
      };
      const expectedBlockchainVersion: ServiceVersionModel = {
        name: 'b-service',
        version: 'a.b.c'
      };
      const expectedCasVersion: ServiceVersionModel = {
        name: 'c-service',
        version: '1.x.c'
      };
      ...
      const serviceInfoSpy = spyOn(
        core['serviceInfo'],
        'getServiceVersion'
      ).and.returnValue(expectedCoreVersion);
      const blockchainSpy = spyOn(
        core['blockchain'],
        'getServiceVersion'
      ).and.returnValue(Promise.resolve(expectedBlockchainVersion));
      const casSpy = spyOn(core['cas'], 'getServiceVersion').and.returnValue(
        Promise.resolve(expectedCasVersion)
      );
      ...
      let fetchedVersions: ServiceVersionModel[] = JSON.parse(
  ...
  describe('handleResolveRequest', () => {
    it('should call the needed functions and return a response', async () => {
      const core = new Core(testConfig, testVersionConfig);
      const mockRequestHandler = jasmine.createSpyObj<IRequestHandler>(
        'versionManagerSpy',
        ['handleResolveRequest']
      );
      mockRequestHandler.handleResolveRequest.and.callFake(() => {
        return resolvedRequest;
      });
      core['versionManager']['getRequestHandler'] = () => {
        return mockRequestHandler;
      };
      core['blockchain']['cachedBlockchainTime'] = {
  ...
  describe('handleOperationRequest', () => {
    it('should call the needed functions and return a response', async () => {
      ...
      mockRequestHandler.handleOperationRequest.and.callFake(() => {
        return resolvedRequest;
      });
      core['versionManager']['getRequestHandler'] = () => {
        return mockRequestHandler;
      ...
      const response = await core.handleOperationRequest(
