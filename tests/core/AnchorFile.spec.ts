...
describe('AnchorFile', async () => {
  describe('parseAndValidate()', async () => {
    ...
    it('should throw if the buffer is not compressed', async () => {
      const anchorFile = {
        ...
        didUniqueSuffixes: [
          'EiA-GtHEOH9IcEEoBQ9p1KCMIjTmTO8x2qXJPb20ry6C0A',
          'EiA4zvhtvzTdeLAg8_Pvdtk5xJreNuIpvSpCCbtiTVc8Ow'
        ]
      };
    ...
    it('should throw if has an unknown property.', async () => {
      ...
      await expectAsync(
        AnchorFile.parseAndValidate(anchorFileCompressed, 2)
      ).toBeRejectedWith(
        new SidetreeError(ErrorCode.AnchorFileHasUnknownProperty)
      );
    });
    ...
    it('should throw if missing batch file hash.', async () => {
      ...
      await expectAsync(
        ...
        new SidetreeError(ErrorCode.AnchorFileBatchFileHashMissing)
      );
    });
    ...
    it('should throw if missing DID unique suffix.', async () => {
      ...
      await expectAsync(
        ...
        new SidetreeError(ErrorCode.AnchorFileDidUniqueSuffixesMissing)
      );
    });
    ...
    it('should throw if batch file hash is not string.', async () => {
      ...
      await expectAsync(
        ...
        new SidetreeError(ErrorCode.AnchorFileBatchFileHashNotString)
      );
    });
    ...
    it('should throw if DID unique suffixes is not an array.', async () => {
      ...
      await expectAsync(
        ...
        new SidetreeError(ErrorCode.AnchorFileDidUniqueSuffixesNotArray)
      );
    });
    ...
    it('should throw if DID unique suffixes is not an array.', async () => {
      ...
      await expectAsync(
        ...
        new SidetreeError(ErrorCode.AnchorFileExceededMaxOperationCount)
      );
    });
    ...
    it('should throw if DID unique suffixes has duplicates.', async () => {
      ...
      await expectAsync(
        ...
        new SidetreeError(ErrorCode.AnchorFileDidUniqueSuffixesHasDuplicates)
      );
    });
    ...
    it('should throw if a DID unique suffix is not string.', async () => {
      ...
      await expectAsync(
        ...
        new SidetreeError(ErrorCode.AnchorFileDidUniqueSuffixEntryNotString)
      );
    });
  ...
  describe('createBufferFromAnchorFileModel', async () => {
    it('should created a compressed buffer correctly.', async () => {
      ...
      const bufferFromCode = await AnchorFile.createBufferFromAnchorFileModel(
        anchorFile
      );

      // Calculated this manually to validate the output
      const expectedEncodedBuffer =
        'H4sIAAAAAAAACqtWSkosSc5wy8xJ9UgszlCyUipLzDFU0lFKyUwJzcssLE0NLk1Ly6xILVayigbJGSnF1gIAyR0qQjUAAAA';
