...
If `ietf-json-patch` is used to add or remove from a proof purpose collection, such as `operations`, `recovery` or `assertionMethod`, per the DID Core spec, a unique `id` property MUST be present.

See [Operation Verification](https://identity.foundation/sidetree/spec/#operation-verification) for more details on how operatios are verified.
