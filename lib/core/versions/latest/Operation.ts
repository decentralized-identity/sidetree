...
export default class Operation {
  ...
  public constructor(operationBuffer: Buffer) {
    ...
    const [decodedHeader, decodedPayload] = Operation.parseAndValidateOperation(
      operation
    );
  ...
  public async verifySignature(publicKey: DidPublicKeyModel): Promise<boolean> {
    const verified = await Jws.verifySignature(
  ...
  private static applyPatchToDidDocument(
    ...
    if (patch.action === 'add-public-keys') {
      ...
      if (didDocument.service !== undefined) {
        service = didDocument.service.find(
  ...
  private static parseAndValidateOperation(
    operation: any
  ): [{ kid: string; operation: OperationType }, any] {
    const decodedProtectedHeadJsonString = Encoder.decodeAsString(
      operation.protected
    );
    ...
    if (headerProperties.length !== 2) {
      throw new SidetreeError(
        ErrorCode.OperationHeaderMissingOrUnknownProperty
      );
    }
  ...
  public static validateCreatePayload(payload: any) {
    ...
    const nextRecoveryOtpHash = Encoder.decodeAsBuffer(
      payload.nextRecoveryOtpHash
    );
  ...
  public static validateRecoverPayload(payload: any) {
    ...
    if (payloadProperties.length !== 6) {
      throw new SidetreeError(
        ErrorCode.OperationRecoverPayloadHasMissingOrUnknownProperty
      );
    }
