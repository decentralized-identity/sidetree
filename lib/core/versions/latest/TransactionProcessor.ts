  ...
  public async processTransaction(
    ...
    try {
      ...
      if (e instanceof SidetreeError) {
        ...
        console.error(
          `Unexpected error processing anchor string, MUST investigate and fix: ${message}`
        );
        return false;
      }
    }
    ...
    console.info(
      `Downloading anchor file '${anchorFileHash}', max size limit ${ProtocolParameters.maxAnchorFileSizeInBytes} bytes...`
    );
    const anchorFileFetchResult = await this.downloadManager.download(
      anchorFileHash,
    ...
    if (anchorFileFetchResult.code === FetchResultCode.NotAFile) {
      console.info(
        `Anchor file hash '${anchorFileHash}' points to a content that is not a file.`
      );
      return true;
    }
    ...
    if (anchorFileFetchResult.code === FetchResultCode.CasNotReachable) {
      console.info(
        `CAS not reachable for anchor file '${anchorFileHash}', will try again later.`
      );
      return false;
    }
    ...
    if (anchorFileFetchResult.code === FetchResultCode.NotFound) {
      console.info(
        `Anchor file '${anchorFileHash}' not found, will try again later.`
      );
      return false;
    }

    console.info(
      `Anchor file '${anchorFileHash}' of size ${
        anchorFileFetchResult.content!.length
      } bytes downloaded.`
    );
    ...
    try {
      ...
      if (error instanceof SidetreeError) {
        ...
        console.info(
          `Anchor file '${anchorFileHash}' failed parsing/validation, transaction '${transaction.transactionNumber}' ignored...`
        );
        return true;
      } else {
        console.error(
          `Unexpected error processing anchor file, MUST investigate and fix: ${error}`
        );
        return false;
      }
    }
    ...
    console.info(
      `Downloading batch file '${anchorFile.batchFileHash}', max size limit ${ProtocolParameters.maxBatchFileSizeInBytes}...`
    );
    const batchFileFetchResult = await this.downloadManager.download(
      anchorFile.batchFileHash,
      ProtocolParameters.maxBatchFileSizeInBytes
    );

    // Nothing to process if the file hash is invalid. No retry needed.
    if (batchFileFetchResult.code === FetchResultCode.InvalidHash) {
      console.info(
        `Batch file '${anchorFile.batchFileHash}' is not a valid hash.`
      );
      return true;
    }
    ...
    if (batchFileFetchResult.code === FetchResultCode.MaxSizeExceeded) {
      console.info(
        `Batch file '${anchorFile.batchFileHash}' exceeded max size limit ${ProtocolParameters.maxBatchFileSizeInBytes}...`
      );
      return true;
    }
    ...
    if (batchFileFetchResult.code === FetchResultCode.NotAFile) {
      console.info(
        `Batch file hash '${anchorFile.batchFileHash}' points to a content that is not a file.`
      );
      return true;
    }
    ...
    if (batchFileFetchResult.code === FetchResultCode.CasNotReachable) {
      console.info(
        `CAS not reachable for batch file '${anchorFile.batchFileHash}', will try again later.`
      );
      return false;
    }
    ...
    if (batchFileFetchResult.code === FetchResultCode.NotFound) {
      console.info(
        `Batch file '${anchorFile.batchFileHash}' not found, will try again later.`
      );
      return false;
    }

    console.info(
      `Batch file '${anchorFile.batchFileHash}' of size ${
        batchFileFetchResult.content!.length
      } downloaded.`
    );
    ...
    try {
      ...
      console.info(error);
      console.info(
        `Batch file '${anchorFile.batchFileHash}' failed parsing/validation, transaction '${transaction.transactionNumber}' ignored.`
      );
      return true;
    }
    ...
    console.info(
      `Processed batch '${anchorFile.batchFileHash}' of ${
        operations.length
      } operations. Time taken: ${endTimer.rounded()} ms.`
    );
