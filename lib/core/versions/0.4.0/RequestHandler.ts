...
export default class RequestHandler implements IRequestHandler {
    ...
    try {
      ...
      switch (operation.type) {
        case OperationType.Create:
          const didDocument = Document.from(
            operation.encodedPayload,
            this.didMethodName,
  ...
  public async handleResolveRequest(
    ...
    try {
      ...
      parameterIsDid = Multihash.isSupportedHash(
        Encoder.decodeAsBuffer(uniquePortion)
      );
    } catch {
      return {
        status: ResponseStatus.BadRequest
  ...
  private async handleResolveRequestWithDid(
    did: string
  ): Promise<ResponseModel> {
  ...
  private async handleResolveRequestWithDidDocument(
    ...
    const currentHashAlgorithm =
      ProtocolParameters.hashAlgorithmInMultihashCode;

    // Validate that the given encoded DID Document is a valid original document.
    const isValidOriginalDocument = Document.isEncodedStringValidOriginalDocument(
      encodedDidDocument,
      ProtocolParameters.maxOperationByteSize
    );
    ...
    const constructedDidDocument = Document.from(
      encodedDidDocument,
      this.didMethodName,
      currentHashAlgorithm
    );
