...
export default class Cryptography {
   ...
   */
  public static async generateKeyPairJwk(
    keyId: string
  ): Promise<[DidPublicKeyModel, PrivateKey]> {
   ...
   * @returns Public key, followed by private key.
   */
  public static async generateKeyPairHex(
    keyId: string
  ): Promise<[DidPublicKeyModel, string]> {
    ...
    try {
      ...
      if (publicKey.publicKeyHex !== undefined) {
        ...
        verified = secp256k1.verify(
          hash,
          Encoder.decodeAsBuffer(encodedSignature),
          publicKeyBuffer
        );
      } else if (publicKey.publicKeyJwk !== undefined) {
        verified = await Secp256k1CryptoSuite.verify(
          content,
          encodedSignature,
