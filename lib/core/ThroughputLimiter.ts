    ...
    for (const transaction of transactions) {
      if (transaction.transactionTime === this.currentBlockHeight) {
        ...
        if (this.currentBlockHeight !== undefined) {
          ...
          const qualifiedTransactionsInCurrentBlock = await transactionSelector.selectQualifiedTransactions(
            this.transactionsInCurrentBlock
          );
          qualifiedTransactions = qualifiedTransactions.concat(
            qualifiedTransactionsInCurrentBlock
          );
        }
