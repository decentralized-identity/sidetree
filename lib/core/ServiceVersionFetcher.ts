...
export default class ServiceVersionFetcher {
  ...
  public async getVersion(): Promise<ServiceVersionModel> {
    ...
    if (
      Date.now() - this.lastTryFetchTime >
      ServiceVersionFetcher.fetchWaitTimeInMilliseconds
    ) {
  ...
  private async tryGetServiceVersion(): Promise<ServiceVersionModel> {
    try {
      ...
      console.info(
        'Trying to get the version info from the blockchain service. Url: ',
        versionUri
      );
      ...
      console.info(
        'Received version response from the blockchain service: ',
        responseBodyBuffer.toString()
      );

      return JSON.parse(responseBodyBuffer.toString());
    } catch (e) {
      console.error(
        'Ignoring the exception during blockchain service version retrieval: %s',
