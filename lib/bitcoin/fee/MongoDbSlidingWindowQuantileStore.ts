 ...
 */
export default class MongoDbSlidingWindowQuantileStore
  implements ISlidingWindowQuantileStore {
  ...
  public constructor(private serverUrl: string, databaseName?: string) {
    this.databaseName = databaseName
      ? databaseName
      : MongoDbSlidingWindowQuantileStore.defaultDatabaseName;
  }
  ...
  public async initialize(): Promise<void> {
    ...
    this.quantileCollection = await MongoDbSlidingWindowQuantileStore.createQuantileCollectionIfNotExist(
      this.db
    );
  }
  ...
  public async get(groupId: number): Promise<QuantileInfo | undefined> {
    return (await this.quantileCollection!.findOne({
  ...
  public async getLastGroupId(): Promise<number | undefined> {
    const lastGroups = await this.quantileCollection!.find()
      .limit(1)
      .sort({ groupId: -1 })
      .toArray();
  ...
  public async getFirstGroupId(): Promise<number | undefined> {
    const firstGroups = await this.quantileCollection!.find()
      .limit(1)
      .sort({ groupId: 1 })
      .toArray();
  ...
  public async removeGroupsGreaterThanEqualTo(groupId: number): Promise<void> {
    await this.quantileCollection!.deleteMany({
      groupId: { $gte: Long.fromNumber(groupId) }
    });
  }
   ...
   */
  private static async createQuantileCollectionIfNotExist(
    db: Db
  ): Promise<Collection> {
    ...
    if (
      collectionNames.includes(
        MongoDbSlidingWindowQuantileStore.quantileCollectionName
      )
    ) {
      ...
      const quantileCollection = await db.createCollection(
        MongoDbSlidingWindowQuantileStore.quantileCollectionName
      );
