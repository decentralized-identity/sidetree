...
export default class BitcoinClient {
  ...
  private async getUnspentOutputs (address: Address): Promise<Transaction.UnspentOutput[]> {
    ...
    // We are setting minimum required confirmation when fetching unspent transactions to 0
    // so that transaction(s) waiting to be confirmed are included. This is allows:
